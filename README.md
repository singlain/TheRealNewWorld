![Pack Header 3](https://cdn.modrinth.com/data/cached_images/89b9de6aad009bf5e56c7ff7508474c511245f5a.png)

<div align="center">

A modpack focused around <font color="#F0DC82">**Create**</font> and its greatest addon - <font color="#57B9FF">**Create Aeronautics**</font>, also bringing many improvements and rebalances to the Vanilla Minecraft - Welcome to <font color="#74c476">**The Real New World!**</font>

</div>

# <font color="#FCD748">Features</font>
- **Create & Create Aeronautics 🚂**
- **Worldgen Overhaul 🌍**
- **New Combat System 🗡️**
- **Progression Rebalances ⚖️**
- **Transportation Rebalances 🐎**
- **Sound & Atmosphere Improvements ✨**
- **...and more! 📌**

Being a spiritual successor of my previous, long forgotten modpack I made for a server of friends - **The Real New World** was remade from ground up for **NeoForge** and had preserved the charm from its predecessor, bringing along with lots of great additions and improvements.

Hundreds of hours went into carefully picking each mod and fine-tuning each config to result in the most balanced and fun experience!

## <font color="#FCD748">Controls ⌨️</font>

- Press <font color="#FCD748">**LSHIFT+R**</font> to toogle the recipe viewer's visibility
- Press <font color="#FCD748">**X**</font> to feint when you're about to swing a weapon 
- Press <font color="#FCD748">**C**</font> to attach a cart onto a mount or yourself
- Press <font color="#FCD748">**V**</font> to toogle slow riding while the cart is attached


## <font color="#FCD748">Create & Create Aeronautics 🚂</font>

<details>
<summary>Details</summary>

These two need no introduction, but in case they do - check out their trailers below.

<details>
<summary>Create Mod Trailer</summary>

<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/rR8W-f9YhYA" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>

</details>
<details>
<summary>Create Aeronautics Mod Trailer</summary>

<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/SO8tpWfZjGk" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>

</details>


</details>


## <font color="#FCD748">New Combat System 🗡️</font>

<details>
<summary>Details</summary>

Ever since 1.9 Minecraft's Combat is mediocre at best despite it being present throughout the entirity of player's gameplay. [Better Combat](https://modrinth.com/mod/better-combat) with a few other subtle additions make combat a bit more engaging and fun.

On top of that, now you can also stack soups, stews and potions to up to 16!

</details>


## <font color="#FCD748">Worldgen Overhaul 🌍</font>

<details>
<summary>Details</summary>

Given that the main focus is on Create and Create Aeronautics means that you as a player, among other things would build a lot of planes, trains, cars and boats, so for that you'd need some plainy land, that's where [Tectonic](https://modrinth.com/datapack/tectonic) comes in - the world now consists of plains and plateaus, giving you much more freedom in places to build or inhabit.

On top of that, for those who love snow, there are also mods to fix the snow generation under trees!

</details>

## <font color="#FCD748">Progression Rebalances ⚖️</font>

<details>
<summary>Details</summary>

Each update makes player's progression in the Survival Mode more and more broken, which I can discuss for hours, but for this modpack I went onto fixing the more apparent problems like the ones I've listed below.

<details>
<summary>Changes</summary>


#### <font color="#FCD748">Villager Trade Rebalance ⚖️</font>

Ever since 1.14 trading in this game has been broken to the point of you being able to obtain the best enchantment in the game within 5 minutes of creating a new world. Mojang tried addressing it with their experiment called "Villager Trade Rebalance" that among other things, locks the most precious enchantments Librarian villagers are selling to be biome-exclusive though they weren't brave enough to make it a part of the game, making it an optional datapack instead. See the table below for the full list:

<details>
<summary>Available tradable enchantments</summary>


| Category | Desert | Plains | Savanna | Snow | Taiga | Jungle | Swamp |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Special | Efficiency III | Protection III | Sharpness III | Silk Touch | Fortune II | Unbreaking II | Mending |
| Common | Fire Protection<br>Thorns<br>Infinity | Punch<br>Smite<br>Bane of Arthropods | Knockback<br>Curse of Binding<br>Sweeping Edge<sup>JE only</sup> | Aqua Affinity<br>Looting<br>Frost Walker | Blast Protection<br>Fire Aspect<br>Flame | Feather Falling<br>Projectile Protection<br>Power | Depth Strider<br>Respiration<br>Curse of Vanishing |

Shoutout to [minecraft.wiki](https://minecraft.wiki/w/Villager_Trade_Rebalance) for composing this table!
</details>

Despite those being the main changes, there are many more subtler ones. Read about them [here.](https://minecraft.wiki/w/Villager_Trade_Rebalance)

#### <font color="#FCD748">Raid Farms Nerf ⚖️</font>

One more change that broke the balance was 1.14's unintentional addition of Raid farms, meaning you can now obtain infinite amounts of Totems of Undying. What was meant to be a special item allowing you to cheat death now is a commodity.

Well, not anymore! While Raid Farms can still grant you saddles, axes and emeralds, they shall no longer drop Totems of Undying, making it special again.

#### <font color="#FCD748">Structure Spawnrates ⚖️</font>

- **Villages** are now 2 times more rare!
- **Pillager Outposts** are now 3 times more rare!
- **Shipwrecks** are now 3 times more rare!
- **Monuments** are now 4 times more rare!

#### <font color="#FCD748">Eye of Ender Recipe ⚖️</font>

Now in order to get to The End you'd not only need to kill a bunch of blazes and endermen, but also visit an Ancient City, a Monument and a Trial Chamber to obtain each of their signature items to craft an Eye of Ender. Good luck!

#### <font color="#FCD748">Easy Anvils & Easy Magic ⚖️</font>

Enchantment and Repair systems of this game were broken ever since they first introduced. While impossible to fully fix without a full overhaul, those are the changes that can make it less annoying while also favoring risk-free repairing your enchanted tools instead of rushing for Mending.

- You now can reroll enchantments at a cost of 5 exp and 1 lapis lazuli
- "Too Expensive!" error when trying to repair / enchant / rename an item is now gone
- Made repair costs per item capped at 8 levels

See more about these two mods [here](https://modrinth.com/mod/easy-magic/) and [there](https://modrinth.com/mod/easy-anvils/)

</details>

</details>

## <font color="#FCD748">Transportation Rebalances 🐎</font>

<details>
<summary>Details</summary>

A heavy focus of this modpack goes into building networks of transportation, so it would only be natural to rebalance almost all of the transportation methods from Vanilla to make them more viable.

### <font color="#FCD748">Ice Boat Nerf 🐎</font>

Ice Boats are now gone. Boats will no longer be able to reach absurd speeds while on ice.

### <font color="#FCD748">Minecarts Buff 🐎</font>

Minecarts powered by power rails will now go at 16 blocks per second instead of the previous 8. Bundle that with the ability to couple them provided by Create and now your mineshafts may shine like never before!

### <font color="#FCD748">Buffed Horses 🐎</font>

This one was given the most care of all.

<details>
<summary>Changes</summary>

#### <font color="#FCD748">Horseman 🐎</font>

Thanks to the [Horseman](modrinth.com/mod/horseman) mod your time with a horse now be much more enjoyable, fixing many issues and giving you the ability to call your horses using Copper Horns! Your horse can now also swim and sit in the same boat with you. Your horse may no longer wander away past 16 block radius from the place it was left in; if it does - the horse now has a slot for a leash within its inventory!

#### <font color="#FCD748">Two Players One Horse 🐎</font>

Same horse can now be ridden by two players!

#### <font color="#FCD748">Horse Breeding Fix 🐎</font>

"Makes so the each stat of a baby horse is atleast as good as the of better parent."

#### <font color="#FCD748">AstikorCarts 🐎</font>

Probably the best of them all - [AstikorCarts](https://modrinth.com/mod/astikorcarts-redux) adds a variety of carts for you and your horse to use, ranging from storage, transportation, crop planting and crop harvesting - those will give your horse a lot more uses!

</details>

</details>

## <font color="#FCD748">Sound and Atmosphere Improvements ✨</font>

<details>
<summary>Details</summary>

### <font color="#FCD748">Sounds ✨</font>

[Sounds](https://modrinth.com/mod/sound) mod adds a ton of sounds to otherwise trivial tasks that are mute by default. See the trailer below to fully see its potential!

<details>
<summary>Sounds Mod Trailer</summary>

<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/9H55Nxoc0uk" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>

</details>

### <font color="#FCD748">Presence Footsteps ✨</font>

People often undersetimate the importance of the footstep sounds in the overall vibe and atmosphere. [Presence Footsteps](https://modrinth.com/mod/presence-footsteps) expands on the footstep sounds, see the trailer below!

<details>
<summary>Presence Footsteps Mod Trailer</summary>

<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/uSZvsO3fyMg" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>

</details>

### <font color="#FCD748">Euphonium ✨</font>

It was rather hard to pick the fitting mod for the ambient sounds, given most of them are either too heavy, too cluttered, or otherwise loud. For this case I've settled on [Euphonium](https://modrinth.com/mod/euphonium), adding just enough ambient sounds to not be too invasive - see the trailer below for a preview.

<details>
<summary>Euphonium Mod Trailer</summary>

<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/sxVLliizM3o" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>

</details>

### <font color="#FCD748">Subtle Effects ✨</font>

[Subtle Effects](https://modrinth.com/mod/subtle-effects) expands the visual side of Minecraft by adding a ton of subtle particle effects and more that you'll surely notice yourself ;)

<details>
<summary>Subtle Effects Mod Trailer</summary>

<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/vzjLaJr7DMQ" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>

</details>

### <font color="#FCD748">Camera Overhaul ✨</font>

[Camera Overhaul](https://modrinth.com/mod/cameraoverhaul) was configured to be much less obvious than it is in its trailer, leaving only a subtle camera tilt when strafing on foot and more obvious one when swimming or riding a mount.

</details>

## <font color="#FCD748">Notes ❗</font>


<details>
<summary>Details</summary>

<details>
<summary>Performance ❗</summary>

Considering running both Create and Create Aeronautics equals running two separate rendering engines at the same time, then having a heavy terrain generation mod on top, plus Distant Horizons would equal a rather abysmal performance regardless of the amount of perfrormance mods shipped alongside it.

So, how can you get around it?

1. **Pausing performance-heavy mods;** luckily, I've already did that for you - by default Distant Horizons will have its distant generation setting set to false and the CPU impact set to low. A big guide on how to tackle Distant Horizons in its current state will be down in the according section of this page.
2. **More RAM;** the bare minumum is at least 4 GB, but it's always best to allocate 8 GB instead. Minecraft is a very weird game RAM-wise and allocating anywhere past 8 either gives no performance benefit or downright worsens the performance.
3. **When playing Singleplayer, running a dedicated server and playing on localhost instead of an integrated server;** it might sound weird, but heavy modpacks like these tend to perform better if you make a server-pack and run in when you want to play the game, joining it via localhost.
4. **Pregenerating chunks;** Distant Horizons has a built-in chunk pregenerator that is described in the guide below. The main TPS eater in Minecraft after complex contraptions and farms is terrain generation. The more you pregenerate, the less stutters and lag you will have in your way. Yeah, a band-aid, I know, but that's just how modded Minecraft is, isn't it?

</details>


<details>
<summary>Distant Horizons 🏞️</summary>

As mentioned before, by default, Distant Horizons' world generator is disabled to reduce the CPU strain and load the LODs and the world gradually. Enabling therefore mentioned world generator will result in poor client performance, poor server performance or both. Below you can see the instuctions for generating LODs and chunks around you in the most efficient way possible.

##### _Please keep in mind that Distant Horizons is still in Beta and **WILL** have __some__ performance issues regardless of how well you tweak it._

1. Find a safe place to AFK in; ideailly in the light-dominant area indoors so the Phantoms and Monsters can't disturb you *
2. Press ESC for the Pause Menu
3. Open the mod menu by pressing **Mods** button
4. Go to the top-left corner and search for **Distant Horizons**
5. Press the **Config** button
6. Crank the CPU Load option to be **5. I Paid For The Whole CPU**
7. Change **Enable Distant Generation** setting to be **True**
8. (Optional) Change **Show Generation Progress** option from **Disabled** to **Overlay** to see the distant generation progress logged on your screen to know when it's finished
9. AFK until the Overlay Text from Distant Horizons' Distant Generator vanishes
10. Follow steps from 2 to 5 once more
11. Change CPU Load option from **5. I Paid For The Whole CPU** to anything from **1. Minimal Impact** to **3. Balanced** depending on how beefy your PC is. **
12. Change "Enable Distant Generation" setting to be **False**
13. (Optional) if you followed step 8, set **Show Generation Progress** setting to "False"
14. Done!

###### * If you're in Singleplayer, you can just wait through the process in the Pause Menu
###### ** Keep in mind, lower CPU Load options will drastically lower the speed of LODs generating and showing up when loading into the world.

<details>
<summary>Servers</summary>

If you're a server owner or have OP rights, you can run commands to use the distant generator regardless of your settings. Keep in mind that your server might experience severe lagging when doing so.

```
/dh pregen start <dimension> <x> <z> <radius in chunks>
```

###### You can find more info about this on the Distant Horizons user guide [here.](https://gitlab.com/distant-horizons-team/distant-horizons/-/wikis/1-user-guide/1-frequently-asked-questions/5-server-owners/Server-Owners)

</details>

</details>

<details>
<summary>Other Notes 📝</summary>

- For the intended experience it is **not recommended** to tinker with configs of any the mods except Distant Horizons; many have seen to cause crashes and inconsistecies when changed and will probably worsen your experience.
- Due to how large this modpack grew, it will take a lot of time to fully flesh it out, so expect bugs, inconsistencies and other issues throughout your journey!
- Before reporting issues on GitHub's Issue Tracker make sure your issue isn't listed in the **Known Issues** section of the latest modpack changelog.
- Modrinth only hosts the latest releases of the modpack, hovewer, you can find all releases ever made on GitHub.
- Balancing is always **ongoing**!

</details>

</details>


## <font color="#FCD748">Credits ⭐</font>

<details>
<summary>Details</summary>

<details>
<summary>Credits</summary>

<details>
<summary>Mods</summary>

- [3d\-Skin\-Layers](https://modrinth.com/mod/zV5r3pPn) by tr7zw
- [AI\-Improvements](https://modrinth.com/mod/DSVgwcji) by BuiltBrokenModding
- [Accelerated Rendering](https://modrinth.com/mod/DfECZO3a) by Argon4W
- [Accurate Block Placement](https://modrinth.com/mod/kzwxhsjp) by Clayborn, Flourick, KadTheHunter
- [Alternate Current](https://modrinth.com/mod/r0v8vy1s) by Space   Walker
- [Amendments](https://modrinth.com/mod/6iTJugQR) by MehVahdJukaar, Plantkillable
- [Architectury](https://modrinth.com/mod/lhGA9TYQ) by shedaniel
- [Armor Trim Item Fix](https://modrinth.com/mod/kIhxG5zE) by MincraftEinstein
- [AstikorCarts Redux](https://modrinth.com/mod/mnLFouB0) by jusipat, MennoMax, pau101, talhanation\# The description of the mod\. This is a simple multiline text string that is used for display purposes in the mod list\.
- [BE Style Wither](https://modrinth.com/mod/OkmRZ5fH) by MORIMORI0317
- [BadOptimizations](https://modrinth.com/mod/g96Z4WVZ) by Thosea
- [Balm](https://modrinth.com/mod/MBAkmtvl) by BlayTheNinth
- [Beautiful Enchanted Books](https://modrinth.com/mod/pcqEicMM) by CerbonXD
- [Better "Add Server"](https://modrinth.com/mod/s4DtR8IM) by Player005
- [Better Advancements](https://modrinth.com/mod/Q2OqKxDG) by way2muchnoise
- [Better Biome Blend](https://modrinth.com/mod/Rs6c7WyL) by FionaTheMortal
- [Better Combat](https://modrinth.com/mod/5sy6g3kz) by Daedelus
- [Better ModList](https://modrinth.com/mod/sbpqhzIG) by foxy, Prospector, haykam821, TerraformersMC
- [Better Ping Display](https://modrinth.com/mod/ZvaHbwoZ) by Quintinity
- [Better Tooltips](https://modrinth.com/mod/Z7rKZJ8k) by isXander, 1foxy2
- [Better World Loading](https://modrinth.com/mod/bmOyRapU) by H1ggsK
- [Bobby](https://modrinth.com/mod/M08ruV16) by johni0702
- [Bookshelf](https://modrinth.com/mod/uy4Cnpcm) by Darkhax
- [CameraOverhaul](https://modrinth.com/mod/m0oRwcZx) by Mirsario
- [Cardboard Chalk Box](https://modrinth.com/mod/WxALA5o1) by seaweedCS
- [Centered Crosshair](https://modrinth.com/mod/xUTHly0N) by JustAlittleWolf
- [Cerulean](https://modrinth.com/mod/dv57xmf9) by Toni, Mephodio
- [Chalk](https://modrinth.com/mod/YWGP4Y1d) by mortuusars
- [Clean F3](https://modrinth.com/mod/5lvjfaRs)
- [Cloth Config v15 API](https://modrinth.com/mod/9s6osm5g) by shedaniel
- [Collective](https://modrinth.com/mod/e0M1UDsY) by Rick South
- [Colorful Hearts](https://modrinth.com/mod/wQfMUdZT) by Terrails
- [Colorwheel](https://modrinth.com/mod/BzHgFoGz) by djefrey
- [Concurrent Chunk Management Engine](https://modrinth.com/mod/COlSi5iR) by RelativityMC, ishland
- [Connector Extras](https://modrinth.com/mod/FYpiwiBR) by Su5eD
- [Continuity](https://modrinth.com/mod/1IjD5062) by PepperCode1
- [Cool Rain](https://modrinth.com/mod/IgftU6Mn) by Leclowndu93150
- [Create](https://modrinth.com/mod/LNytGWDc) by simibubi
- [Create Aeronautics](https://modrinth.com/mod/oWaK0Q19) by Simulated Team
- [Create: Blocks & Bogies](https://modrinth.com/mod/j4ARnQwY) by Weido's Oddities \(@weidos\.oddities\), teacup \(@t\_\_cup\), Matt
- [Create: Copycats\+](https://modrinth.com/mod/UT2M39wf) by Lysine, Bennyboy1695, Redcat\_XVIII
- [Create: EMI Schematics](https://modrinth.com/mod/5bgqgixq) by LiukRast, swzo
- [Create: Pattern Schematics](https://modrinth.com/mod/cpqKG67r) by Cake
- [Cubes Without Borders](https://modrinth.com/mod/ETlrkaYF) by Kir\_Antipov
- [Default Options](https://modrinth.com/mod/WEg59z5b) by BlayTheNinth
- [Delete Worlds To Trash](https://modrinth.com/mod/7diJQfvM) by Fuzs
- [Distant Horizons](https://modrinth.com/mod/uCdwusMi)
- [Dynamic FPS](https://modrinth.com/mod/LQ3K71Q1) by juliand665 & LostLuma
- [EMI](https://modrinth.com/mod/fRiHVvU7) by Emi
- [Easy Anvils](https://modrinth.com/mod/OZBR5JT5) by Fuzs
- [Easy Magic](https://modrinth.com/mod/9hx3AbJM) by Fuzs
- [Eating Animation](https://modrinth.com/mod/rUgZvGzi) by theone\_ss, spusik\_, PinkGoosik, DoctorNight1
- [Enchanted Bookshelves](https://modrinth.com/mod/CIfCvYLI) by boyonk
- [Enhanced Attack Indicator](https://modrinth.com/mod/eTy17BBS) by Minenash
- [EntityCulling](https://modrinth.com/mod/NNAgCjsB) by tr7zw
- [Euphonium](https://modrinth.com/mod/XH8cnjHW) by svenhjol, Sisby folk
- [Fabric Language Kotlin](https://modrinth.com/mod/Ha28R6CL) by FabricMC
- [Fast Minecart](https://modrinth.com/mod/BHUz7jn9) by Me\!
- [Fast Paintings](https://modrinth.com/mod/z3TzcquW) by MehVahdJukaar
- [FastQuit](https://modrinth.com/mod/x1hIzbuY) by KingContaria
- [Ferrite Core](https://modrinth.com/mod/uXXizFIs) by malte0811
- [Flerovium](https://modrinth.com/mod/4Rh1Mobu) by MoePus
- [Forgified Fabric API](https://modrinth.com/mod/Aqlf1Shp) by Sinytra, FabricMC
- [Fzzy Config](https://modrinth.com/mod/hYykXjDp) by fzzyhmstrs
- [GPUTape](https://modrinth.com/mod/5CTSrY8X) by mr\_toad
- [Gnetum](https://modrinth.com/mod/7MoE34WK) by decce
- [Hide Experimental Warning](https://modrinth.com/mod/Rm4OOdHd) by Rick South
- [Horse Breeding Fix](https://modrinth.com/mod/4QFMVWw4) by misterxycb
- [Horseman](https://modrinth.com/mod/qIv5FhAA) by mortuusars
- [Ice Boat Nerf](https://modrinth.com/mod/Udjno5eL) by supersaiyansubtlety
- [Iceberg](https://modrinth.com/mod/5faXoLqX) by Grend
- [Illagers Wear Armor](https://modrinth.com/mod/xXNP32XJ) by TallestEgg
- [ImmediatelyFast](https://modrinth.com/mod/5ZwdcRci) by RK\_01
- [Immersive Optimization](https://modrinth.com/mod/vNZgQmjg) by Luke100000
- [Iris](https://modrinth.com/mod/YL57xq9U) by coderbot, IMS212
- [Item Borders](https://modrinth.com/mod/b1fMg6sH) by Grend
- [Ixeris](https://modrinth.com/mod/p8RJPJIC) by decce
- [Kotlin for Forge](https://modrinth.com/mod/ordsPcFz)
- [Krypton FNP](https://modrinth.com/mod/JkxWVYwU) by 404, tuxed
- [LambDynamicLights](https://modrinth.com/mod/yBW8D80W) by LambdAurora
- [Leave My Bars Alone](https://modrinth.com/mod/gK9mebQg) by Fuzs
- [Legendary Tooltips](https://modrinth.com/mod/atHH8NyV) by Grend
- [Lithium](https://modrinth.com/mod/gvQqBUqZ) by 2No2Name, JellySquid
- [Lithostitched](https://modrinth.com/mod/XaDC71GB) by Apollo
- [Log Begone](https://modrinth.com/mod/9ON3zv6e) by AzureDoom
- [M\.R\.U](https://modrinth.com/mod/SNVQ2c0g) by IMB11 \(mineblock11\)
- [MC\-273265 Fix](https://modrinth.com/mod/L1BFjX2g) by Singlain
- [Main Menu Credits](https://modrinth.com/mod/qJDfP7WN) by isXander
- [Map Tooltip](https://modrinth.com/mod/GNtptqqU) by VendoAU
- [MaxHealthFix](https://modrinth.com/mod/mH8wdmqr) by Darkhax
- [MidnightLib](https://modrinth.com/mod/codAaoxh) by TeamMidnightDust, Motschen
- [MmmMmmMmmMmm](https://modrinth.com/mod/Adega8YN) by Mehvahdjukaar, Bonusboni, Gooigipunch, Plantkillable
- [Model Gap Fix](https://modrinth.com/mod/QdG47OkI) by MehVahdJukaar
- [ModernFix](https://modrinth.com/mod/nmDcB62a) by embeddedt
- [MonoLib](https://modrinth.com/mod/9leXt4A5) by Lupin, Jason13
- [Moonlight Lib](https://modrinth.com/mod/twkfQtEc) by MehVahdJukaar
- [More Culling](https://modrinth.com/mod/51shyZVL) by FX \- PR0CESS, 1Foxy2
- [More Sounds](https://modrinth.com/mod/8jvcOd6S) by DVOA
- [Mouse Tweaks](https://modrinth.com/mod/aC3cM3Vq) by Ivan Molodetskikh \(YaLTeR\)
- [Neo Bee Fix](https://modrinth.com/mod/DzSY371i) by WFPhantom
- [NetherPortalFix](https://modrinth.com/mod/nPZr02ET) by BlayTheNinth
- [No Chat Reports](https://modrinth.com/mod/qQyHxfxd) by Aizistral
- [No Chat Restrictions](https://modrinth.com/mod/z440MEwJ) by Aizistral
- [No Resource Pack Warnings Forge](https://modrinth.com/mod/cWtswsPK) by 1foxy2
- [Noisium](https://modrinth.com/mod/hasdd01q) by Steveplays28
- [Not Enough Recipe Book](https://modrinth.com/mod/bQh7xzFq) by OctoStudios
- [NotEnoughAnimations](https://modrinth.com/mod/MPCX6s5C) by tr7zw
- [OctoLib](https://modrinth.com/mod/RH2KUdKJ) by OctoStudios
- [OptiGUI](https://modrinth.com/mod/JuksLGBQ) by opekope2
- [PacketFixer](https://modrinth.com/mod/c7m1mi73) by TonimatasDEV
- [Palladium](https://modrinth.com/mod/JNRr4jji) by Mr\.Toad
- [Particle Core](https://modrinth.com/mod/RSeLon5O) by fzzyhmstrs
- [Particle Effects](https://modrinth.com/mod/PLAGcSFJ) by LopyMine, KlashRaick
- [Particle Rain](https://modrinth.com/mod/nrikgvxm) by pigcart
- [Paxi](https://modrinth.com/mod/CU0PAyzb) by YUNGNICKYOUNG
- [Player Animator](https://modrinth.com/mod/gedNE4y2) by KosmX
- [Preferred Gamerules](https://modrinth.com/mod/kKib77nY) by Estecka
- [Presence Footsteps](https://modrinth.com/mod/rcTfTZr3) by Hurricaaane \(Ha3\), Sollace
- [PrickleMC](https://modrinth.com/mod/aaRl8GiW) by Darkhax
- [Prism](https://modrinth.com/mod/1OE8wbN0) by Grend
- [Puzzles Lib](https://modrinth.com/mod/QAGBst4M) by Fuzs
- [QuickBench](https://modrinth.com/mod/v2yISwb0) by Ampflower, Tfarecnim, Shadows\-of\-Fire
- [Recipe Fixer](https://modrinth.com/mod/Nmc13D4j) by Mysticpasta1, TonimatasDEV
- [Recreative](https://modrinth.com/mod/EBWdtRFr) by Evan
- [Reese's Sodium Options](https://modrinth.com/mod/Bh37bMuy) by FlashyReese
- [Resourcefulconfig](https://modrinth.com/mod/M1953qlQ)
- [Responsive Knockback](https://modrinth.com/mod/FMQ7tQOa) by Revvilo
- [Responsive Shields](https://modrinth.com/mod/MBmh0f9A) by Revvilo
- [Sable](https://modrinth.com/mod/T9PomCSv) by RyanHCode
- [Saturn](https://modrinth.com/mod/2eT495vq) by AbdElAziz
- [Scholar](https://modrinth.com/mod/fX4dIQCo) by mortuusars
- [Screenshot to Clipboard](https://modrinth.com/mod/1KiJRrTg) by comp500
- [Server Pinger Fixer](https://modrinth.com/mod/iqK5uv72) by JustAlittleWolf
- [ShulkerBoxTooltip](https://modrinth.com/mod/2M01OLQq) by MisterPeModder
- [Simple Snowy Fix](https://modrinth.com/mod/3WP3HZRG) by KostromDan, Apollo
- [Sinytra Connector](https://modrinth.com/mod/u58R1TMW)
- [Skeleton Aiming Fix](https://modrinth.com/mod/afi6lIuM) by Xiaolin2
- [Smart Particles](https://modrinth.com/mod/l8rWP4Tc) by Andrew\-Chedid
- [Smarter Farmers](https://modrinth.com/mod/Bh6ZOMvp) by MehVahdJukaar
- [Smooth Skies](https://modrinth.com/mod/uvlgIwBD) by Microcontrollers
- [Smooth Swapping](https://modrinth.com/mod/ydZic5r4) by Schauweg
- [Snow Under Trees](https://modrinth.com/mod/Q3vyMuj2) by bl4ckscor3
- [Sodium](https://modrinth.com/mod/AANobbMI) by JellySquid \(jellysquid3\), IMS212
- [Sodium Extra](https://modrinth.com/mod/PtjYWJkn) by FlashyReese
- [Sodium Shadowy Path Blocks](https://modrinth.com/mod/EIa1eiMm) by Rynnavinx
- [Sounds](https://modrinth.com/mod/ZouiUX7t) by IMB11, cassiancc
- [SparseStructures](https://modrinth.com/mod/qwvI41y9) by MaxenceDC
- [Stackable Potions](https://modrinth.com/mod/yppxDTPT) by CursedFlames
- [Stackable Stew and Soup](https://modrinth.com/mod/zqFS0wQJ) by Lupin, Jason13
- [Structure Layout Optimizer](https://modrinth.com/mod/ayPU0OHc) by TelepathicGrunt
- [Stylish Effects](https://modrinth.com/mod/onDuQF5e) by Fuzs
- [Subtle Effects](https://modrinth.com/mod/4q8UOK1d) by MincraftEinstein
- [Tectonic](https://modrinth.com/mod/lWDHr9jE) by Apollo
- [Tiny Item Animations](https://modrinth.com/mod/wMkevcSR) by Trivaxy
- [Tiny Skeletons](https://modrinth.com/mod/A2xcMSUn) by Fuzs
- [ToadLib](https://modrinth.com/mod/CYQ7VYrM) by Mr\.Toad
- [TooManyRecipeViewers](https://modrinth.com/mod/yFypjcfd) by Nolij \(@xdMatthewbx\#1337\) & the Craftoria team
- [TwoPlayersOneHorse](https://modrinth.com/mod/c89BhsB1) by Beethoven92
- [TxniLib](https://modrinth.com/mod/vBbPDuOs) by Toni
- [Updating World Icon](https://modrinth.com/mod/ZlX3EVmE) by CursedFlames
- [Void Totem](https://modrinth.com/mod/q6eiiQ07) by Affehund
- [XP\_Stream](https://modrinth.com/mod/QZqv3OuH)
- [YUNG's API](https://modrinth.com/mod/Ua7DFN59) by YUNGNICKYOUNG
- [YetAnotherConfigLib](https://modrinth.com/mod/1eAoo2KR) by isXander
- [aero\_copycats](https://modrinth.com/mod/wjpmYU1u)
- [lazy\-language\-loader](https://modrinth.com/mod/Nz0RSWrF) by ChachyDev
- [quick\-pack](https://modrinth.com/mod/pSISfJ4O) by Drex

</details>

<details>
<summary>Datapacks</summary>

- [NoRaidTotems](https://modrinth.com/datapack/noraidtotems) by Rochedo
- [Nullscape](https://modrinth.com/datapack/nullscape) by Stardust Labs
- [Progressive Ender Eyes](https://modrinth.com/datapack/progressive-eyes) by Ranold
- [Totem of Undying Death Messages](https://modrinth.com/datapack/totemdeathmessagesbon) by n_bonnie2
- [Craftable Enchanted Golden Apples](https://vanillatweaks.net/) by Vanilla Tweaks

</details>

<details>
<summary>Resource Packs</summary>

- [Vanilla Tweaks](https://vanillatweaks.net/) by Vanilla Tweaks Team
- [Recolorful Containers GUI + HUD](https://modrinth.com/resourcepack/recolourful-containers-gui) by Vi_Tul
- [Enhanced Boss Bars](https://modrinth.com/resourcepack/enhanced-boss-bars) by Naku
- [C418 Songs Only](https://modrinth.com/resourcepack/c418-songs-only) by redsquare
- [Big Lush Cave Panorama](https://modrinth.com/resourcepack/big-lush-cave-panorama) by Singlain (credited myself award)
- [Big Lush Cave Panorama (Shaders)](https://modrinth.com/resourcepack/big-lush-cave-panorama-(shaders)) by Singlain (credited myself again award)
- [(Bee's) Fancy Crops](https://modrinth.com/resourcepack/fancy-crops) by bebebea_loste

</details>

<details>
<summary>Other Mods</summary>

> The following mods were bundled as .jar(s) in the overrides due to them being unavailable on Modrinth/GitHub or being CurseForge exclusives.

- [Create/Sable LambDynamicLights]([https://www.curseforge.com/minecraft/mc-mods/placebo](https://www.curseforge.com/minecraft/mc-mods/create-sable-lambdynamiclights))

</details>

</details>

### <font color="#FCD748">Special Thanks ⭐</font>
- @GlitchCMD
  - General Help, Playtesting, Feedback
- @RalseiFromDeltarune
  - General Help, Updating the mods/configs, Troubleshooting, Playtesting, Feedback

</details>

